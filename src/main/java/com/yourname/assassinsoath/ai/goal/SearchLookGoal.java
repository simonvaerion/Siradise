...
public class SearchLookGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (state.isEmpty() || state.get().stage() < StealthAwarenessTracker.STAGE_HEAD_TURN) {
            return false;
        }
    ...
    private void directLook() {
        Optional<StealthAwarenessTracker.AwarenessState> state = StealthAwarenessTracker.get(mob);
        float detection = state.map(StealthAwarenessTracker.AwarenessState::detection).orElse(0.0f);
        float spread = Mth.lerp(detection, 4.0f, 0.6f);
        Vec3 jitter = new Vec3(
                mob.getRandom().nextDouble() * spread - spread / 2.0,
                mob.getRandom().nextDouble() * spread / 3.0 - spread / 6.0,
                mob.getRandom().nextDouble() * spread - spread / 2.0);
