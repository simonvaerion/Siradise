...
public final class StealthSoundEvents {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        STEP_COOLDOWN.computeIfPresent(id, (uuid, cooldown) -> cooldown > 0 ? cooldown - 1 : null);
    ...
    private static void emitNoise(ServerLevel level, Player player, double radius, float baseIntensity) {
        ...
        List<Mob> mobs = level.getEntitiesOfClass(Mob.class, box, mob -> mob.isAlive() && !mob.isAlliedTo(player));
        for (Mob mob : mobs) {
            ...
            detection = Mth.clamp(detection, 0.0f, 1.0f);
            if (detection < 0.25f) {
                continue;
            }
            StealthAwarenessTracker.addNoise(mob, player, origin, detection);
            StealthAwarenessTracker.get(mob).ifPresent(state -> StealthChannel.CHANNEL.send(
